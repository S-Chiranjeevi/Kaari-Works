  ...
  return <div className="page-shell">
    ...
    {tab === "discover" && <>
      ...
      <section className="section" id="catalogue"><div className="section-head"><div><h2>Find handmade goods</h2><p>Explore craft from independent makers across India.</p></div><label className="search"><Search size={17}/><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search products, crafts, materials" /></label></div><div className="filters">{categories.map((item) => <button key={item} className={`filter ${category === item ? "active" : ""}`} onClick={() => setCategory(item)}>{item}</button>)}</div>
        <div className="product-grid">{loading ? <div className="empty">Loading the artisan catalogue…</div> : shownProducts.length === 0 ? <div className="empty">No products yet. Be the first artisan to publish a listing.</div> : shownProducts.map((product) => <article className="product-card" key={product.id}><div className="product-art">{product.image_url ? <img src={product.image_url} alt={product.name} /> : <span aria-hidden="true">{product.category.toLowerCase().includes("pottery") ? "🏺" : product.category.toLowerCase().includes("jewel") ? "💍" : product.category.toLowerCase().includes("wood") ? "🪵" : "🧵"}</span>}<span className="category-label">{product.category}</span></div><div className="product-body"><h3>{product.name}</h3><div className="seller-name">Made by {product.seller_name || "Independent artisan"}</div><p className="product-description">{product.description || "A handmade piece created by an independent artisan."}</p><div className="product-foot"><div className="product-price">₹{product.price_inr.toLocaleString("en-IN")}<small>per piece · min. {product.minimum_order_quantity}{product.lead_time ? ` · ${product.lead_time}` : " units"}</small></div><button className="button" onClick={() => { setContact(product); setContactQuantity(String(product.minimum_order_quantity)); setContactMessage(`Hello, I'm interested in ${product.name}. Please share bulk pricing and availability.`); }}>Contact seller</button></div></div></article>)}</div>
      </section>
    </>}

    {tab === "sell" && <main className="seller-layout"><div className="eyebrow">For independent artisans</div><h1>Put your craft in front of buyers.</h1><p>Share product details, set a fair price, and receive wholesale enquiries directly.</p><SignedOut><div className="form-card"><h2>Sign in to create your artisan shop</h2><p>Use your Clerk account to publish product listings and reply to buyers.</p><SignInButton mode="modal"><button className="button">Sign in to continue</button></SignInButton></div></SignedOut><SignedIn><form className="form-card" onSubmit={publishListing}><div className="form-grid"><label className="field"><span>Product name</span><input required minLength={2} maxLength={180} value={form.name} onChange={(e) => setField("name", e.target.value)} placeholder="e.g. Handwoven cotton table runner" /></label><label className="field"><span>Craft category</span><select value={form.category} onChange={(e) => setField("category", e.target.value)}>{categories.slice(1).map((item) => <option key={item}>{item}</option>)}</select></label><label className="field full"><span>Tell buyers about the product</span><textarea required maxLength={5000} value={form.description} onChange={(e) => setField("description", e.target.value)} placeholder="Materials, technique, story, dimensions, care…" /></label><label className="field"><span>Price per piece (₹)</span><input required type="number" min="1" value={form.price} onChange={(e) => setField("price", e.target.value)} /></label><label className="field"><span>Making cost (₹)</span><input type="number" min="0" value={form.cost} onChange={(e) => setField("cost", e.target.value)} /></label><label className="field"><span>Hours to make</span><input type="number" min="0" step="0.5" value={form.hours} onChange={(e) => setField("hours", e.target.value)} /></label><label className="field"><span>Years in this craft</span><input type="number" min="0" value={form.experience} onChange={(e) => setField("experience", e.target.value)} /></label><label className="field"><span>Available quantity</span><input type="number" min="0" value={form.quantity} onChange={(e) => setField("quantity", e.target.value)} /></label><label className="field"><span>Minimum bulk order</span><input required type="number" min="1" value={form.minimum} onChange={(e) => setField("minimum", e.target.value)} /></label><label className="field"><span>Lead time</span><input value={form.leadTime} onChange={(e) => setField("leadTime", e.target.value)} placeholder="e.g. 2–3 weeks" /></label><label className="field"><span>Upload product photo (JPG, PNG, WebP · up to 1 MB)</span><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(e) => uploadImage(e.target.files?.[0])} /></label></div>{form.image && <div className="image-editor"><img src={form.image} alt="Product image preview" /><div className="image-actions"><button type="button" className="button soft" onClick={() => void enhanceImage()} disabled={enhancing}>{enhancing ? "Enhancing with Gemini…" : "Enhance with Gemini"}</button><button type="button" className="button outline" onClick={() => setField("image", "")}>Remove photo</button></div><p className="form-note">Gemini can alter visual details. Compare with the original and publish only an accurate representation.</p></div>}{suggestedRange && <div className="price-suggestion"><strong>Indicative dynamic price range: ₹{suggestedRange.low.toLocaleString("en-IN")}–₹{suggestedRange.high.toLocaleString("en-IN")} per piece</strong><span>Recalculated from making cost, estimated labour at ₹75/hour, a capped experience premium, overhead and margin. This is a cost-based estimate, not live market data.</span><button type="button" className="button soft" onClick={() => setField("price", String(suggestedRange.low))}>Use lower suggested price</button></div>}<p className="form-note"><Sparkles size={13} /> Gemini enhancement requires your own GEMINI_API_KEY in web/.env.local.</p><button className="button" type="submit" disabled={saving}>{saving ? "Publishing…" : "Publish listing"}</button></form></SignedIn></main>}
    ...
    <footer className="footer">Kaari Works · Handmade, directly from artisans</footer>
